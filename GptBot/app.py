...
openai.api_key = "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"
def limit_words(text, word_limit=4500):
    words = text.split()
    return " ".join(words[:word_limit]) if len(words) > word_limit else text

def extract_text_from_pdf(file, word_limit=4000):
    reader = PyPDF2.PdfReader(file)
    full_text = "".join([page.extract_text() for page in reader.pages])
    return limit_words(full_text, word_limit)

def extract_text_from_docx(file, word_limit=4000):
    doc = docx.Document(file)
    full_text = "".join([para.text for para in doc.paragraphs])
    return limit_words(full_text, word_limit)


def get_answer_from_gpt(question,type, document_text, max_response_tokens=2000):
    prompt = f"The document contains the following information:\n\n{document_text}\n\nUser's question: {question}\nAnswer:"

    chat_response = openai.chat.completions.create(
        model="gpt-3.5-turbo",  # or "gpt-3.5-turbo" for cost efficiency or gpt-4o"
        messages=[
            {"role": "system",
             "content": f"You are an {type} expert who helps with answering questions based on documents. Please provide only relevant facts and figures, if you do not know just let me know."},
